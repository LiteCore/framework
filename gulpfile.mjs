...
gulp.task('less-backend', function() {
  ...
  gulp.src('public_html/backend/template/less/variables.less')
	.pipe(less())
    .on('error', function (err) {
      console.error('LESS Error:', err.message);
      this.emit('end'); // Prevents Gulp from stopping
  ...
  return gulp.src(['public_html/backend/template/less/*.less', '!**/variables.less'])
    .pipe(sourcemaps.init())
    .pipe(less())
    .on('error', function (err) {
      console.error('LESS Error:', err.message);
...
gulp.task('less-frontend', function() {
  ...
  gulp.src('public_html/frontend/template/less/variables*.less')
    .pipe(less())
    .on('error', function (err) {
      console.error('LESS Error:', err.message);
      this.emit('end'); // Prevents Gulp from stopping
