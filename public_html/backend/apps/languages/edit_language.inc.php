	...
	if (isset($_POST['save'])) {
		...
		try {
			...
			if (empty($_POST['set_store']) && isset($language->data['code']) && $language->data['code'] == settings::get('site_language_code') && $language->data['code'] != $_POST['code']) {
				throw new Exception(language::translate('error_cannot_rename_site_language', 'You must change the site language before renaming it.'));
			}
			...
			if (!empty($_POST['set_store']) && empty($_POST['status']) && isset($language->data['code']) && $language->data['code'] == settings::get('site_language_code')) {
				throw new Exception(language::translate('error_cannot_set_disabled_site_language', 'You cannot set a disabled language as site language.'));
			}
		...
		<?php echo functions::form_begin('language_form', 'post', false, false, 'style="max-width: 640px;"'); ?>
			...
			<div class="row">
				<div class="form-group col-md-6">
					...
					<div class="btn-group btn-block btn-group-inline" data-toggle="buttons">
						<label class="btn btn-default<?php echo (isset($_POST['status']) && $_POST['status'] == 1) ? ' active' : ''; ?>"><?php echo functions::form_input_radio_button('status', '1', true); ?> <?php echo language::translate('title_enabled', 'Enabled'); ?></label>
						<label class="btn btn-default<?php echo (isset($_POST['status']) && $_POST['status']  == -1) ? ' active' : ''; ?>"><?php echo functions::form_input_radio_button('status', '-1', true); ?><?php echo language::translate('title_hidden', 'Hidden'); ?></label>
						<label class="btn btn-default<?php echo empty($_POST['status']) ? ' active' : ''; ?>"><?php echo functions::form_input_radio_button('status', '0', true); ?><?php echo language::translate('title_disabled', 'Disabled'); ?></label>
					</div>
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="form-group col-md-6">
					...
					<div class="btn-group btn-block btn-group-inline" data-toggle="buttons">
					<label class="btn btn-default<?php echo (!isset($_POST['direction']) || $_POST['direction'] == 'ltr') ? ' active' : ''; ?>" style="text-align: left;"><?php echo functions::form_input_radio_button('direction', 'ltr', true); ?> <?php echo language::translate('title_left_to_right', 'Left To Right'); ?></label>
					<label class="btn btn-default<?php echo (isset($_POST['direction']) && $_POST['direction'] == 'rtl') ? ' active' : ''; ?>" style="text-align: right;"><?php echo functions::form_input_radio_button('direction', 'rtl', true); ?><?php echo language::translate('title_right_to_left', 'Right To Left'); ?></label>
					</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="form-group col-md-6">
					...
					<div class="btn-group btn-block btn-group-inline" data-toggle="buttons">
					<label class="btn btn-default<?php echo (!empty($_POST['url_type']) && $_POST['url_type'] == 'none') ? ' active' : ''; ?>"><?php echo functions::form_input_radio_button('url_type', 'none', !empty($_POST['url_type']) ? true : 'none'); ?> <?php echo language::translate('title_none', 'None'); ?></label>
					<label class="btn btn-default<?php echo (!empty($_POST['url_type']) && $_POST['url_type'] == 'path') ? ' active' : ''; ?>"><?php echo functions::form_input_radio_button('url_type', 'path', true); ?> <?php echo language::translate('title_path_prefix', 'Path Prefix'); ?></label>
					<label class="btn btn-default<?php echo (!empty($_POST['url_type']) && $_POST['url_type'] == 'domain') ? ' active' : ''; ?>"><?php echo functions::form_input_radio_button('url_type', 'domain', true); ?> <?php echo language::translate('title_domain', 'Domain'); ?></label>
					</div>
				</div>
