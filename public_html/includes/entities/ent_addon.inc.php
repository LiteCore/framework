<?php
	...
	class ent_addon {
		...
		public function load($id) {
			...
			$this->data['name'] = $dom->getElementsByTagName('name')->item(0)->textContent ?? '';
			$this->data['version'] = $dom->getElementsByTagName('version')->item(0)->textContent ?? date('Y-m-d', filemtime($this->data['location'] .'vmod.xml'));
