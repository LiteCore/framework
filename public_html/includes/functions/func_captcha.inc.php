<?php
	...
	function captcha_draw($id='default', $config=[], $parameters='') {
		...
		return implode(PHP_EOL, [
			...
			'  <span class="input-group-text" style="padding: 0; line-height: 0;"><img src="data:image/gif;base64,'. $base64_image .'" alt="" style="width: '. $config['width'] .'px; height: '. $config['height'] .'px;"'. ($parameters ? ' ' . $parameters : '') .'></span>',
			'  ' . form_input_text('lc-captcha-response', '', 'required autocomplete="off" style="font-size: '. (int)$config['font_size'] .'px; padding: 0; text-align: center;" maxlength="'. strlen($code) .'"'. ($parameters ? ' '. $parameters : '')),
			'</div>',
		]);
	}
