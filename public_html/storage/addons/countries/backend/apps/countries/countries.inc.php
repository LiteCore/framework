		...
		try {
			...
			foreach ($_POST['countries'] as $country_code) {
				...
				if (!empty($_POST['disable']) && $country_code == settings::get('site_country_code')) {
					throw new Exception(t('error_cannot_disable_site_country', 'You cannot disable the store country'));
				}
		...
		<table class="table data-table">
			...
			<tbody>
				...
				<tr class="<?php echo empty($country['status']) ? 'semi-transparent' : ''; ?>">
					...
					<td><a class="btn btn-default btn-sm" href="<?php echo document::href_ilink(__APP__.'/edit_country', ['country_code' => $country['iso_code_2']]); ?>" title="<?php echo t('title_edit', 'Edit'); ?>"><?php echo f::draw_fonticon('edit'); ?></a></td>
				</tr>
				<?php } ?>
			</tbody>
			...
			<tfoot>
				<tr>
					<td colspan="9"><?php echo t('title_countries', 'Countries'); ?>: <?php echo language::number_format($num_rows); ?></td>
				</tr>
			</tfoot>
		</table>
