			...
			<div class="grid">
				...
				<div class="col-lg-6">
					...
					<table class="table data-table">
						...
						<tfoot>
							<tr>
								<td colspan="4"><a class="add btn btn-default" href="#"><?php echo f::draw_fonticon('add'); ?> <?php echo t('title_add_zone', 'Add Zone'); ?></a></td>
							</tr>
						</tfoot>
					</table>
				</div>
...
<script>
	...
	$('form[name="country_form"] .add').on('click', function(e) {
		...
		let $output = $([
			'<tr>',
			'  <td><?php echo f::escape_js(f::form_input_hidden('zones[new_zone_index][id]', '')); ?></td>',
			'  <td><?php echo f::escape_js(f::form_input_text('zones[new_zone_index][code]', '')); ?></td>',
			'  <td><?php echo f::escape_js(f::form_input_text('zones[new_zone_index][name]', '')); ?></td>',
