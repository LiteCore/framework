		...
		buildEditor: function () {
			...
			t.$ed
				...
				.on('input', function (e) {
					...
					if (
						typeof event === 'object' &&
						(event.inputType === 'insertReplacementText' || (event.inputType === 'insertText' && event.data === null))
					) {
						t.$c.trigger('tbwchange');
					}
				})
			...
			var t = this,
				...
				$btn = $('<button/>', {
					...
					title:
						...
						(btn.key ? ' (' + (t.isMac ? 'Cmd' : 'Ctrl') + ' + ' + btn.key + ')' : ''),
					tabindex: -1,
					mousedown: function () {
						...
						if (
							(t.$btnPane.hasClass(prefix + 'disable') || t.$box.hasClass(prefix + 'disabled')) &&
